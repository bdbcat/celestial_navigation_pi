...
double astrolabe::util::dms_to_d(int deg, int min, double sec) {
  ...
  */
  ...
  if (deg < 0 || min < 0 || sec < 0) result = -result;
  return result;
}

void astrolabe::util::ecl_to_equ(double longitude, double latitude,
                                 double obliquity, double& ra, double& dec) {
...
void astrolabe::util::equ_to_horiz(double H, double decl, double& A,
                                   double& h) {
...
void astrolabe::util::equ_to_ecl(double ra, double dec, double obliquity,
...
double astrolabe::util::interpolate3(double n, const vector<double>& y) {
  ...
  */
  if (n < -1 || n > 1)
    throw Error(
        "astrolabe::util::interpolate3: interpolating factor out of range");
...
double astrolabe::util::interpolate_angle3(double n, const vector<double>& y) {
  ...
  */
  if (n < -1 || n > 1)
    throw Error(
        "astrolabe::util::interpolate_angle3: interpolating factor out of "
        "range");
...
public:
  Lex(ifstream& infile) {
    ...
    while (infile) {
      ...
      for (std::vector<string>::const_iterator p = fields.begin();
           p != fields.end(); ++p) {
        const string str = *p;
        if (str[0] == '#') break;
...
void astrolabe::util::load_params() {
  ...
  */
  ...
  if (!fname) fname = "astrolabe_params.txt";
  ifstream infile(fname);
  if (!infile)
    throw Error(
        "astrolabe::util::load_params: unable to open param file. Either set "
        "ASTROLABE_PARAMS correctly or create astrolabe_params.txt in the "
        "current directory");
  ...
  while (!token.empty()) {
    if (token == "standard_timezone_name")
      ...
      if (unit != "day" && unit != "days" && unit != "hour" &&
          unit != "hours" && unit != "minute" && unit != "minutes" &&
          unit != "second" && unit != "seconds")
        throw Error(
            "astrolabe::util::load_params: bad value for "
            "standard_timezone_offset units");
      ...
      if (direction != "east" && direction != "west")
        throw Error(
            "astrolabe::util::load_params: longitude direction must be 'west' "
            "or 'east'");
      if (direction == "east") xlongitude = -xlongitude;
      ...
      if (direction != "north" && direction != "south")
        throw Error(
            "astrolabe::util::load_params: latitude direction must be 'north' "
            "or 'south'");
      if (direction == "south") xlatitude = -xlatitude;
      ...
      throw Error("astrolabe::util::load_params: unknown token '" + token +
                  "' at line " + int_to_string(lex.lineno) + " in param file");
    token = lex.get_token();
  }
}
...
double astrolabe::util::polynomial(const vector<double>& terms, double x) {
  ...
  */
  ...
  double result = *p;
  for (++p; p != terms.rend(); ++p) result = result * x + *p;
  return result;
}
